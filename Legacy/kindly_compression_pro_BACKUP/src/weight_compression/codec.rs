...
impl StructuredSparseWeightCodec {
    ...
    fn prune_structured_blocks(
        ...
        if sparsity <= 0.0 || sparsity >= 1.0 {
            return Err(CompressionError::InvalidSparsity);
        }
    ...
    fn quantize_q6_6(&self, block: &SparseBlock) -> QuantizedBlock {
    ...
    fn quantize_q8_8(&self, block: &SparseBlock) -> QuantizedBlock {
    ...
    fn decompress_from_dictionary(
        ...
        for (i, &centroid_id) in compressed.centroid_ids.iter().enumerate() {
            ...
            if centroid_id as usize >= self.block_centroids.len() {
                return Err(CompressionError::DictionaryLookupFailed);
            }
