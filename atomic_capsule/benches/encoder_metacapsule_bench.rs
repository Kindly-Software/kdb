//! Av1EncoderMetacapsule B32 Benchmarks - Performance Validation
//!
//! [TRADE SECRET] Comprehensive performance benchmarking for lockfree AV1 encoder orchestration.
//!
...
//! - **Conservative**: 2-5× speedup target (profiling + lockfree)
//! - **Optimistic**: 10-20× speedup target (full SIMD + tile parallelism)
...
fn benchmark_state_transitions(c: &mut Criterion) {
    ...
    group.sample_size(1000); // 1000+ iterations per B32
...
fn benchmark_phase_completion(c: &mut Criterion) {
    ...
    group.bench_function("complete_bitstreamwrite", |b| {
        let metacapsule = create_test_metacapsule();
        b.iter(|| {
            black_box(metacapsule.complete_phase(EncoderPhase::BitstreamWrite));
            metacapsule.reset_phases();
        })
    });
...
// ============================================================================
// BENCHMARK GROUP 6: Phase Reset
// ============================================================================

fn benchmark_phase_reset(c: &mut Criterion) {
    let mut group = c.benchmark_group("phase_reset");
    group.sample_size(1000);

    group.bench_function("reset_phases", |b| {
        let metacapsule = create_test_metacapsule();
        b.iter(|| {
            ...
            metacapsule.complete_phase(EncoderPhase::DctTransform);
            // Reset all phases
            black_box(metacapsule.reset_phases());
        })
    });
...
fn benchmark_full_workflow_simulation(c: &mut Criterion) {
    ...
    group.sample_size(100); // Fewer samples for complex workflows

    group.bench_function("full_encode_cycle", |b| {
        let metacapsule = create_test_metacapsule();
        b.iter(|| {
            // Full encode workflow (state machine + phase tracking)
            let _ = metacapsule.transition_state(EncoderState::Idle, EncoderState::Lookahead);
            metacapsule.complete_phase(EncoderPhase::Lookahead);
    ...
    group.bench_function("intra_only_workflow", |b| {
        let metacapsule = create_test_metacapsule();
        b.iter(|| {
            // I-frame workflow (minimal phases)
            let _ = metacapsule.transition_state(EncoderState::Idle, EncoderState::Lookahead);
            metacapsule.complete_phase(EncoderPhase::Lookahead);
