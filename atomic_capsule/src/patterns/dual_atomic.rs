...
//!
//! **UCE33 Tier 1 Atomic Capsule pattern from The Atomic Capsule architecture.**
//!
...
//! - `#ASSUME_TOCTOU_SAFE`: Generation counter prevents races (line 312-335)
...
impl DualAtomicU64 {
    ...
    // ```rust
    // // Generation counter pattern (lines 318-335)
    ...
    //
    // See tests/dual_atomic_concurrent_property_tests.rs for correct patterns.
    ...
    ///
    /// let dual = DualAtomicU64::new(0, 0);
    /// dual.store_primary(100, Ordering::Release);
    ...
    /// assert_eq!(dual.load_primary(Ordering::Relaxed), 20);
    /// ```
    #[inline(always)]
    ...
    ///
    /// let dual = DualAtomicU64::new(10, 0);
    /// let old = dual.fetch_add_primary(5, Ordering::SeqCst);
    /// assert_eq!(old, 10);
    /// assert_eq!(dual.load_primary(Ordering::Relaxed), 15);
    /// ```
    #[inline(always)]
    ...
    /// use core::sync::atomic::Ordering;
    ///
    /// let dual = DualAtomicU64::new(20, 0);
    /// let old = dual.fetch_sub_primary(5, Ordering::SeqCst);
    /// assert_eq!(old, 20);
    /// assert_eq!(dual.load_primary(Ordering::Relaxed), 15);
    /// ```
    #[inline(always)]
    ...
    /// use core::sync::atomic::Ordering;
    ///
    /// let dual = DualAtomicU64::new(0, 99);
    /// assert_eq!(dual.load_secondary(Ordering::Relaxed), 99);
    /// ```
    #[inline(always)]
    ...
    ///
    /// let dual = DualAtomicU64::new(0, 0);
    /// dual.store_secondary(200, Ordering::Release);
    ...
    /// - #VERIFY_ORDERING_SUFFICIENT: Property test line 208-284 validates
    ...
    ///
    /// let dual = DualAtomicU64::new(0, 0);
    /// let gen1 = dual.increment_secondary(Ordering::SeqCst);
    /// let gen2 = dual.increment_secondary(Ordering::SeqCst);
    /// assert_eq!(gen1, 0);
    /// assert_eq!(gen2, 1);
    /// assert_eq!(dual.load_secondary(Ordering::Relaxed), 2);
    /// ```
    #[inline(always)]
    ...
    /// let result = dual.compare_exchange_secondary(
    ///     5, 10,
    ///     Ordering::SeqCst, Ordering::Relaxed
    /// );
    /// assert_eq!(result, Ok(5));
    /// assert_eq!(dual.load_secondary(Ordering::Relaxed), 10);
    /// ```
    #[inline(always)]
    ...
    /// let old = dual.fetch_add_secondary(50, Ordering::SeqCst);
    /// assert_eq!(old, 100);
    /// assert_eq!(dual.load_secondary(Ordering::Relaxed), 150);
    /// ```
    #[inline(always)]
