//! Generic Work-Stealing Queue<T> (Tier 1 Atomic + Tier 4 Batch)
//!
//! **100% Lockfree** bounded work-stealing queue using dual-atomic head/tail with generation counters.
//! Generic over any `T: Send + Sized`, suitable for parallel processing of structured data.
//!
//! ## Architecture
//!
...
//! - **Item Storage**: Fixed ring buffer with MaybeUninit<T> (deterministic memory)
...
//!
//! #ASSUME_BOUNDED_CAPACITY: Fixed capacity prevents unbounded memory growth
//! #VERIFY_BOUNDED_CAPACITY: Return Err(QueueFullError) on push when full
//!
//! ## Usage Example
//!
//! ```rust
...
//!
//! // Create queue with 1024 capacity
...
/// Generic lockfree work-stealing queue with bounded capacity and cache-line alignment
///
...
/// - Bytes 128+: Ring buffer (capacity slots)
///
...
/// - Compound speedup: 3-10× from lockfree coordination
///
...
pub struct WorkStealingQueue<T> {
    ...
    /// Ring buffer: fixed capacity slots (MaybeUninit until pushed)
    /// Box ensures heap allocation for large T
    buffer: Box<[UnsafeCell<MaybeUninit<T>>]>,
}

// Safety: WorkStealingQueue<T> is Send if T is Send
...
// Safety: WorkStealingQueue<T> is Sync if T is Send (shared access is safe)
...
impl<T> WorkStealingQueue<T> {
    /// Create new work-stealing queue with specified capacity
    ///
    /// Memory layout:
    /// - Head: 64B cache line (consumer-local, LIFO)
    /// - Tail: 64B cache line (shared, FIFO)
    /// - Ring: capacity slots (heap-allocated)
    ///
    /// # Arguments
    ///
    /// * `capacity` - Maximum number of items (must be power of 2 for efficiency)
    ///
    /// # Panics
    ///
    ...
    pub fn push(&self, item: T) -> Result<(), QueueFullError> {
        ...
        let next_idx = (head_idx + 1) & (self.capacity - 1);
    ...
    pub fn pop(&self) -> Option<T> {
        loop {
            ...
            let prev_idx = (head_idx.wrapping_sub(1)) & (self.capacity - 1);
    ...
    pub fn steal(&self) -> Option<T> {
        loop {
            ...
            let next_idx = (tail_idx + 1) & (self.capacity - 1);
    ...
    /// Get queue capacity (maximum number of items)
    ///
    /// # Examples
    ///
    /// ```rust
