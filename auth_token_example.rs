//! Standalone example for AuthTokenCapsule - No external dependencies
//! This demonstrates the T1 Atomic capsule works correctly
...
fn test_concurrent() {
    println!("Test 2: Concurrent Access (8 threads × 100 validations)));
    println!("{}","-".repeat(60);
    ...
    assert_eq!(hits as usize, num_threads * iterations_per_thread, "Cache hits mismatch));
    println!("✓ PASS));
    println!();
}
...
fn test_performance() {
    ...
    if latency_ns < 100.0 {
        println!("✓ PASS (target: <100ns)));
    } else {
        println!("⚠ Close to target (actual: {:.1}ns)", latency_ns);
    }
    println!();
}
...
fn test_layout() {
    ...
    assert_eq!(size, 128, "Size mismatch));
    assert_eq!(alignment, 128, "Alignment mismatch));
    ...
    assert_eq!(offset, 0, "Runtime alignment mismatch));
