...
use crate::layout::{standard64, DefaultLayout, Layout, LayoutRaw, Standard64};
#[cfg(feature = "std")]
use crate::telemetry::TelemetrySample;

#[cfg(all(feature = "auto_tune", feature = "compact48"))]
use crate::layout::unpack_q6_10;
#[cfg(feature = "auto_tune")]
use crate::layout::unpack_q8_8;
#[cfg(feature = "compact48")]
use crate::layout::Compact48;
...
impl LayoutKind {
    ...
    pub const fn default() -> Self {
        #[cfg(feature = "standard64")]
        {
            Self::Standard64
        }

        #[cfg(all(not(feature = "standard64"), feature = "compact48"))]
        {
            Self::Compact48
        }
    }
...
impl AtomicBreakerSWeMR {
    ...
    pub fn apply_sample(&self, sample: &TelemetrySample) {
        ...
        match self.layout_kind() {
            LayoutKind::Standard64 => {
                let word = self.load_relaxed();
                let mu_q = crate::layout::pack_q8_8(clamped.mu_norm);
                let sg_q = crate::layout::pack_q8_8(clamped.sg_norm);
                let backoff = clamped
                    .backoff_hint
            ...
            LayoutKind::Compact48 => {
                let mu_q = crate::layout::pack_q6_10(clamped.mu_norm);
                let sg_q = crate::layout::pack_q6_10(clamped.sg_norm);
...
#[cfg(all(test, feature = "std", feature = "standard64"))]
mod tests {
    use super::*;
    use crate::cause;
    ...
    fn apply_sample_updates_metrics() {
        ...
        let guard = AtomicBreakerGuard::new(breaker.load_relaxed());
        assert_eq!(guard.err(), 2);
        assert!(guard.mu_norm() >= crate::layout::pack_q8_8(1.5));
        assert!(guard.sg_norm() >= crate::layout::pack_q8_8(0.75));
        assert_eq!(guard.cause() & cause::LAT, cause::LAT);
        assert_eq!(guard.backoff(), 7);
    }

    #[cfg(all(feature = "mpmc", feature = "standard64"))]
    #[test]
...
#[cfg(all(test, feature = "std", feature = "compact48"))]
mod compact_tests {
    use super::*;
