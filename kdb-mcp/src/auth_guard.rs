...
impl AuthGuard {
    ...
    pub fn authenticate(
        ...
        // ====================================================================
        use crate::anomaly_detector::AnomalyDetectorCapsule;
        ...
        let anomaly_risk = if let Some(history) = request_history {
            ...
            match features {
                Ok(f) => match self.anomaly_detector.predict_anomaly(&f) {
                    Ok(prediction) => {
                        if prediction.is_anomalous {
                            (prediction.anomaly_score * 100.0) as u32  // Convert to 0-100 scale
                        } else {
                            0
                        }
                    }
                    Err(_) => 0,  // On error, assume no anomaly
                }
                Err(_) => 0,
            }
        } else {
...
impl Default for AuthGuard {
    fn default() -> Self {
        Self {
            ...
            #[cfg(feature = "tls")]
            tls: Arc::new(TlsCapsule::new(
                std::path::Path::new("/tmp/cert.pem"),
                std::path::Path::new("/tmp/key.pem"),
                "example.com"
            ).unwrap_or_else(|_| panic!("TLS initialization failed in Default"))),
            ...
            #[cfg(feature = "tls")]
            acme_cert_manager: Arc::new(AcmeCertManagerCapsule::new(
                "example.com",
                std::path::Path::new("/tmp/acme/cert.pem"),
                std::path::Path::new("/tmp/acme/key.pem"),
            ).unwrap_or_else(|_| panic!("ACME cert manager initialization failed in Default"))),
