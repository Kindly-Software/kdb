...
impl TotpValidatorCapsule {
    ...
    /// Validate TOTP code with clock skew tolerance
    ///
    ...
    ///
    /// **Performance**: O(1), ~50ns (3 HMAC-SHA1 computations)
    ...
    pub fn validate_totp(
        ...
        if code == expected_current {
            ...
            self.last_totp_time.store(current_step, Ordering::Release);
            self.successful_validations.fetch_add(1, Ordering::Relaxed);
            return Ok(true);
        }

        // Check ±1 time step for clock skew tolerance
        // Previous window (30 seconds ago)
        if current_step > 0 {
            let expected_prev = self.compute_totp_code(&secret.secret, current_step - 1)?;
            if code == expected_prev {
                let last_time = self.last_totp_time.load(Ordering::Acquire);
                if last_time == current_step - 1 {
                    self.replay_attacks_detected.fetch_add(1, Ordering::Release);
                    self.failed_validations.fetch_add(1, Ordering::Relaxed);
                    return Err(TotpError::CodeReused);
                }
                self.last_totp_time.store(current_step - 1, Ordering::Release);
                self.successful_validations.fetch_add(1, Ordering::Relaxed);
                return Ok(true);
            }
        }

        // Next window (30 seconds from now)
