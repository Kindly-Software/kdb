...
//! Every command is logged with CRC64 hash for tamper detection.
//!
...
///
/// # Performance
/// - log_command(): ~100ns (CRC64 + atomic append)
...
/// - #ASSUME_CRC64_COLLISION_FREE: <1 in 2^64 collision probability
...
impl AuditLogCapsule {
    ...
    const CRC64_POLY: u64 = 0x42F0E1EBA9EA3693; // Standard CRC64 polynomial
    ...
    fn compute_hash(&self, prev_hash: u64, command: &str, timestamp: u64) -> u64 {
        let mut crc = prev_hash ^ 0xFFFFFFFFFFFFFFFF;

        // Hash previous hash
        for &byte in prev_hash.to_le_bytes().iter() {
            crc = self.crc64_update(crc, byte);
        }
    ...
    /// CRC64 byte update (ECMA polynomial)
    #[inline]
    fn crc64_update(&self, crc: u64, byte: u8) -> u64 {
        let mut crc = crc ^ (byte as u64);
        for _ in 0..8 {
            if crc & 1 != 0 {
                crc = (crc >> 1) ^ Self::CRC64_POLY;
            } else {
                crc >>= 1;
            }
        }
...
impl ComplianceMetadata {
    ...
    pub fn new() -> Self {
        Self {
            ...
            hash_algorithm: "CRC64-ECMA".to_string(),
            chain_valid: true,
