...
pub fn Button(
    ...
    let (bg_color, text_color, border_radius) = match variant {
        ButtonVariant::Primary => (COLOR_GOLD_BRIGHT, COLOR_BLACK, RADIUS_MD),
        ButtonVariant::Secondary => (COLOR_BYZANTINE_ROYAL, COLOR_WHITE, RADIUS_MD),
        ButtonVariant::Light => (COLOR_LIGHT_GRAY, COLOR_BYZANTINE_DEEP, RADIUS_MD),
        ButtonVariant::Tertiary => ("transparent", COLOR_BYZANTINE_ROYAL, RADIUS_SM),
    };

    // Size-specific styles
    let (padding, font_size) = match size {
        ButtonSize::Small => (format!("{} {}", SPACING_2, SPACING_3), FONT_SIZE_SM),
        ButtonSize::Medium => (format!("{} {}", SPACING_3, SPACING_6), FONT_SIZE_BASE),
        ButtonSize::Large => (format!("{} {}", SPACING_4, SPACING_8), FONT_SIZE_LG),
    };
