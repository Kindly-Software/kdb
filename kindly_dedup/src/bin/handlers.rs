...
use kindly_dedup::{
    generate_synthetic_corpus_with_stats, DedupPipeline, StreamingDedupPipeline,
...
pub fn handle_dedup(args: &DedupArgs, global: &GlobalArgs) -> Result<()> {
    ...
    if !global.quiet {
        ...
        if args.universal {
            println!("Mode:       UniversalDedupPipeline (T6 Mixed, O(1) 222 MB) [DEFAULT]");
        } else {
            println!("Mode:       Legacy DedupPipeline (--legacy flag)");
    ...
    // Route to appropriate pipeline implementation (UniversalDedupPipeline is DEFAULT as of v3.0)
    // If NOT explicitly requesting legacy mode, use Universal
    if args.universal {
        return handle_dedup_universal(args, global);
    }
...
// ============================================================================
// Universal Dedup Pipeline Handler (T6 Mixed, O(1) 222 MB memory)
// ============================================================================

fn handle_dedup_universal(args: &DedupArgs, global: &GlobalArgs) -> Result<()> {
    ...
    if !global.quiet {
        println!("Creating UniversalDedupPipeline (T6 Mixed Orchestrator)...");
        println!("  Cores: {} (CPU detected)", num_cores);
        println!();
    }
    ...
    // Step 2: Create UniversalDedupPipeline
    ...
    let mut pipeline = UniversalDedupPipeline::new(
        ...
        num_docs,
        args.threshold,
        0,                  // start_doc_id: process from beginning
        num_docs as u64,    // end_doc_id: process all documents
    )
    .context("Failed to create UniversalDedupPipeline")?;

    if !global.quiet {
        println!("  Orchestrator initialized: {} ms", start.elapsed().as_millis());
        println!();
        println!("Processing corpus (5-phase atomic state machine)...");
    }

    // Step 3: Process corpus (atomic state machine: Read→Sign→Hash→Cluster→Output)
    let start = Instant::now();
    if let Err(e) = pipeline.process_corpus() {
        return Err(anyhow::anyhow!("Failed to process corpus: {:?}", e));
    }
    let process_time = start.elapsed();
    ...
    if !global.quiet {
        ...
        println!("  Processed {} documents in {:.2}s ({:.0} docs/sec)\n",
                 num_docs, process_time.as_secs_f64(), throughput);
        println!("Finding duplicate clusters...");
    }

    // Step 4: Find duplicates
    let find_start = Instant::now();
    let clusters_u64 = pipeline
        .find_duplicates()
        .context("Failed to find duplicate clusters")?;
    let find_time = find_start.elapsed();

    if !global.quiet {
        println!("  Found {} clusters in {:.2}s\n", clusters_u64.len(), find_time.as_secs_f64());
        println!("Writing output to {}...", args.output.display());
    }

    // Convert clusters from Vec<Vec<u64>> to Vec<Vec<usize>>
    let clusters: Vec<Vec<usize>> = clusters_u64
        .into_iter()
        .map(|cluster| cluster.into_iter().map(|id| id as usize).collect())
        .collect();
    ...
    if !global.quiet {
        println!();
        println!("✓ Deduplication complete! (T6 Mixed Architecture)");
        println!("  Total time:   {:.2}s", total_time.as_secs_f64());
        println!("  Throughput:   {:.0} docs/sec", num_docs as f64 / total_time.as_secs_f64());
        println!("  Clusters:     {}", clusters.len());
        println!("  Memory (O(1)): 222 MB constant (independent of corpus size)");
        println!("  Output:       {}", args.output.display());
    }
...
fn show_dedup_help() {
    ...
    println!("  --universal               Use UniversalDedupPipeline (T6 Mixed, O(1) 222MB)");
