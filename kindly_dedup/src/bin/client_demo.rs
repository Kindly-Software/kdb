...
fn print_help() {
    ...
    println!();
    println!("EXAMPLES:");
    println!("  # Run standard 3-tier demo");
    println!("  client_demo");
    println!();
    println!("  # Run on custom data with default threshold");
    println!("  client_demo --custom-data my_corpus.txt");
    println!();
    println!("  # Run with custom threshold and save results");
    println!("  client_demo --custom-data corpus.txt --threshold 0.90 --output results.json");
    println!();
...
fn run_scale_tier(
    ...
    #[cfg(feature = "meta-capsule")]
    {
        ...
        let _ = log_security_event(
            ...
            None,
            0,
            &format!("Starting {}: {} scale ({} docs)", tier_num, tier_name, doc_count),
        );
    }
    ...
    #[cfg(feature = "meta-capsule")]
    {
        ...
        let _ = log_security_event(
            ...
            0,
            &format!(
                "Completed {}: {} docs in {:.2}s, {:.0} docs/sec, {} clusters",
                tier_num,
                doc_count,
                pipeline_time.as_secs_f64(),
                throughput,
...
fn run_custom_data_tier(
    ...
    println!("Loading custom data from: {}", file_path);
    let corpus = load_custom_corpus(file_path, Some(progress.clone()))
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(file_path) = args.custom_data_path {
        ...
        // Run custom data deduplication
        let results = run_custom_data_tier(&file_path, args.threshold, &cpu_caps)?;
        ...
        let scale_100m = if sys_caps.can_run_tier4 {
            println!("\n═══════════════════════════════════════════════════════════");
