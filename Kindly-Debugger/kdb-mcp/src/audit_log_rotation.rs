...
impl AuditLogRotationCapsule {
    ...
    fn rotate_internal(&self, log_path: &Path) -> Result<PathBuf> {
        ...
        let file_stem = log_path.file_stem()
            .ok_or_else(|| AuditRotationError::PathError(
                format!("Log path has no file stem: {}", log_path.display())
            ))?
            .to_str()
            .ok_or_else(|| AuditRotationError::PathError(
                format!("Log path contains invalid UTF-8: {}", log_path.display())
            ))?;
    ...
    fn cleanup_old_logs(&self, log_path: &Path) -> Result<()> {
        ...
        for entry in fs::read_dir(log_dir)? {
            ...
            let filename = path.file_name()
                .and_then(|n| n.to_str())
                .ok_or_else(|| AuditRotationError::PathError(
                    format!("Invalid filename in log directory: {}", path.display())
                ))?;
