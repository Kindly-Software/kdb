...
impl SharedStateCapsule {
    ...
    pub fn new(path: Option<&Path>) -> io::Result<Self> {
        ...
        // Safety: File descriptor is valid, length is constant SIZE (4MB)
        // #ASSUME_MMAP_VALID: memmap2::MmapMut guarantees valid mapping on success
        // #ASSUME_FILE_PERMISSIONS: File created with RW permissions, not shared via symlink attack
        // #VERIFY: Unit tests validate mmap creation and file access patterns
        let mut mmap = unsafe {
            MmapOptions::new()
                .len(Self::SIZE)
                .map_mut(&file)?
    ...
    ///
    /// # Safety
    /// #ASSUME_LAYOUT_ALIGNMENT: SharedStateLayout is #[repr(C, align(64))] matching mmap alignment
    /// #ASSUME_MMAP_INITIALIZED: mmap is valid and initialized (checked in new())
    /// #VERIFY: compile-time size assertion validates layout correctness
    #[cfg(target_os = "linux")]
    ...
    ///
    /// # Safety
    /// #ASSUME_EXCLUSIVE_ACCESS: &mut MmapMut ensures exclusive access to mutable header
    /// #ASSUME_LAYOUT_ALIGNMENT: SharedStateLayout is #[repr(C, align(64))]
    /// #VERIFY: Borrow checker prevents concurrent header_mut calls
    #[cfg(target_os = "linux")]
    ...
    ///
    /// # Safety
    /// #ASSUME_SESSION_MUT_INDEX_VALID: index < SESSION_CAPACITY (enforced: range check)
    /// #ASSUME_EXCLUSIVE_ACCESS: &mut self ensures no concurrent mutable access
    /// #VERIFY: Borrow checker prevents concurrent session_entry_mut calls
    #[cfg(target_os = "linux")]
