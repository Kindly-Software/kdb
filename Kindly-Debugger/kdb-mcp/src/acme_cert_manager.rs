...
impl AcmeCertManagerCapsule {
    ...
    /// Create a dummy ACME cert manager for testing (no actual cert/key files required).
    ///
    /// **Use Case**: Test environments where actual Let's Encrypt certificates are not available.
    /// **WARNING**: DO NOT use in production. For testing and Default impl only.
    ///
    /// **Performance**: 0ns (const initialization)
    ///
    /// **Behavior**:
    /// - Sets dummy expiry to 90 days from now
    ...
    pub fn new_dummy() -> Self {
        ...
        let expiry = now_unix + (90 * 24 * 60 * 60); // 90 days from now

        let mut domain = [0u8; 64];
        let domain_str = b"test.local";
        domain[..domain_str.len()].copy_from_slice(domain_str);

        let mut cert_path = [0u8; 128];
        let cert_path_str = b"/tmp/acme/cert.pem";
        cert_path[..cert_path_str.len()].copy_from_slice(cert_path_str);

        let mut key_path = [0u8; 128];
        let key_path_str = b"/tmp/acme/key.pem";
        key_path[..key_path_str.len()].copy_from_slice(key_path_str);
