        ...
        for (i, entry) in self.tools.iter().enumerate() {
            if entry.tool_id.load(Ordering::Relaxed) == 0 {
                ...
                if entry.tool_id.compare_exchange(
                    ...
                    // Copy tool name
                    // Safety: We have exclusive access to this slot via successful CAS on tool_id
                    // #ASSUME_EXCLUSIVE_SLOT_ACCESS: CAS success grants exclusive write access to entry
                    // #ASSUME_NAME_LEN_VALID: name_bytes.len() <= TOOL_NAME_LEN (enforced by caller)
                    // #VERIFY: Concurrent registration tests validate atomicity (tests::test_concurrent_*)
...
impl ToolHandle {
    ...
    ///
    /// # Safety
    /// #ASSUME_ENTRY_LIFETIME: entry pointer is valid for lifetime of ToolHandle
    /// #ASSUME_ENTRY_IMMUTABLE: ToolEntry is never deallocated while registry exists
    /// #VERIFY: Registry is 'static or lives as long as all ToolHandle instances
