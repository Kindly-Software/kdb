...
Complete documentation for all 16 MCP tools provided by kindly-debugger.
...
Execute an XPath query against XML documents.

**Parameters**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `document` | string | Yes | Path to XML document or document ID |
| `query` | string | Yes | XPath expression |
| `namespaces` | object | No | Namespace prefix mappings |
...
```json
{
  ...
  "params": {
    "name": "xpath_query",
    "arguments": {
      "document": "/path/to/config.xml",
      "query": "//server[@env='production']"
    }
  }
}
```
...
```json
{
  ...
  "result": {
    "content": [
      {
        "type": "text",
        "text": "Query returned 2 nodes:\n\n<server env=\"production\" host=\"api.example.com\"/>\n<server env=\"production\" host=\"web.example.com\"/>"
      }
    ]
  }
}
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| `document` | string | Yes | Path to XML document |
| `schema` | string | Yes | Path to XSD schema |
...
Preload documents into cache for faster queries.

**Parameters**

| Name | Type | Required | Description |
|------|------|----------|-------------|
| `documents` | array | Yes | List of document paths to preload |
